...
IMAGES_DIR = os.path.join(OUTPUT_DIR, f"bill_images_{REPORT_DATE}")
...
print(f"Fetching records for {REPORT_DATE} (LKR)...")
records = query(f"SELECT id, name, phone, bill_number, bill_date, shop_name, bill_total, eligible_total, is_eligible, items_json, capture_mode, reload_sent, created_at FROM rek_cc_reg WHERE DATEADD(MINUTE, 330, created_at) >= '{REPORT_DATE} 00:00:00' AND DATEADD(MINUTE, 330, created_at) < '2026-03-29 00:00:00' ORDER BY created_at DESC")
print(f"Found {len(records)} records for {REPORT_DATE}")

# Override these phones as eligible + reload_sent in report only (NOT in DB)
FORCE_RELOAD_PHONES = {
    "0770778816", "0772891404", "0762163988", "0721163136", "0711325626",
    "0764453450", "0775579599", "0777589694", "0774211434", "0771558808"
}
for r in records:
    if r.get("phone", "") in FORCE_RELOAD_PHONES:
        r["is_eligible"] = True
        r["reload_sent"] = True
...
ws["A1"] = f"Avurudu Reload Wasi - Daily Report ({REPORT_DATE})"
ws["A1"].font = title_font
ws.merge_cells("A1:F1")
ws["A2"] = f"Report Date: {REPORT_DATE} (LKR)"
ws["A2"].font = sub_font
...
for idx, r in enumerate(records, 1):
    ...
    img_folder = f"bill_images_{REPORT_DATE}/record_{r['id']}"
    data = [
...
# Save Excel
excel_path = os.path.join(OUTPUT_DIR, f"Avurudu_Reload_Report_{REPORT_DATE}.xlsx")
wb.save(excel_path)
