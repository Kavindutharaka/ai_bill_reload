...
IMAGES_DIR = os.path.join(OUTPUT_DIR, "bill_images")
...
# Fetch all records (no images)
print("Fetching records...")
records = query("SELECT id, name, phone, bill_number, bill_date, shop_name, bill_total, eligible_total, is_eligible, items_json, capture_mode, reload_sent, created_at FROM rek_cc_reg ORDER BY created_at DESC")
...
ws["A1"] = "Avurudu Reload Wasi - Daily Report"
ws["A1"].font = title_font
ws.merge_cells("A1:F1")

today_lkr = (datetime.utcnow() + LKR_OFFSET).strftime("%Y-%m-%d %H:%M")
ws["A2"] = f"Generated: {today_lkr} (LKR)"
ws["A2"].font = sub_font
...
for idx, r in enumerate(records, 1):
    ...
    img_folder = f"bill_images/record_{r['id']}"

    data = [
        idx,
        r.get("name", ""),
        r.get("phone", ""),
        to_lkr(r.get("created_at", "")),
        r.get("bill_number", ""),
        r.get("shop_name", "") or "-",
        r.get("bill_total", 0),
        ...
        (r.get("capture_mode") or "single").upper(),
        img_folder
    ]
...
for r in records:
    ...
    data = [
        ...
        r.get("shop_name", "") or "-",
        r.get("eligible_total", 0),
        "SENT" if r.get("reload_sent") else "PENDING",
        elig_str
    ]
    for col, val in enumerate(data, 1):
...
# Save Excel
today_str = (datetime.utcnow() + LKR_OFFSET).strftime("%Y-%m-%d")
excel_path = os.path.join(OUTPUT_DIR, f"Avurudu_Reload_Report_{today_str}.xlsx")
wb.save(excel_path)
